...
st.set_page_config(page_title="Inventario Pro + Conversión USD", layout="wide")
...
# --- FUNCIONES DE APOYO (FINANZAS E IMÁGENES) ---

@st.cache_data(ttl=3600)
def obtener_tc():
    """Obtiene tipo de cambio real de USD a MXN."""
    try: 
...
def calcular_costo_final_mxn(usd_bruto, tc_mercado):
    """Lógica exacta solicitada: (USD * Tax) * TC + Comisión."""
    usd_tax = usd_bruto * 1.0825  # Tax 8.25%
    comi_mxn = (usd_tax * 0.12) * 19  # Comisión 12% con TC fijo de 19
...
def lectura_inventario():
    # Incluimos Pagado Venta en las columnas base
    ...
    try: 
        ...
        for col in columnas_base:
            if col not in df.columns:
                if col == "Pagado Venta": df[col] = False
                elif col in ["Precio MXN", "Precio Venta", "Cantidad", "Vendidos"]: df[col] = 0.0
                else: df[col] = ""
...
# Conversión de tipos para cálculos de visualización
...
with st.sidebar:
    ...
    # 2. Lógica de Costo USD (Funcional)
    st.markdown("---")
    ...
    if f_foto: st.image(f_foto, use_container_width=True)
    ...
    with st.form("registro_inv", clear_on_submit=False):
        ...
        with col_f1:
            ...
            f_cantidad = st.text_input("Stock", value="1")
        with col_f2:
            f_talla = st.text_input("Talla")
            f_pagado = st.checkbox("¿Pagado?")
            ...
            if f_nombre and costo_mxn_final > 0:
                with st.spinner("Guardando..."):
                    ...
                    nuevo = pd.DataFrame([{
                        ...
                        "Precio MXN": costo_mxn_final, # Aquí entra la lógica USD->MXN
                        "Precio Venta": limpiar_num(f_precio_venta),
                        "Color": f_color, "Talla": f_talla, 
                        "Cantidad": limpiar_num(f_cantidad), 
                        "Vendidos": limpiar_num(f_vendidos),
                        "Pagado Venta": f_pagado, "Imagen": url_foto
                    }])
    ...
    # --- BORRADO CON CONFIRMACIÓN (INTEGRADO) ---
    st.divider()
    if not df_inv.empty:
        ...
        if st.session_state.get('confirmar_borrado_inv', False):
            ...
            if c1.button("SÍ", use_container_width=True):
                ...
                cols_clean = ["Producto", "Tienda", "Precio MXN", "Precio Venta", "Color", "Talla", "Cantidad", "Vendidos", "Pagado Venta", "Imagen"]
...
edited_inv = st.data_editor(
    ...
    column_config={
        ...
        "Pagado Venta": st.column_config.CheckboxColumn("PAGADO?"),
...
if st.button("💾 GUARDAR CAMBIOS DE TABLA"):
    cols_save = ["Producto", "Tienda", "Precio MXN", "Precio Venta", "Color", "Talla", "Cantidad", "Vendidos", "Pagado Venta", "Imagen"]
